...
@csrf_exempt
def artcle_column(request):
    if request.method == "GET":
        columns = ArticleColumn.objects.filter(user=request.user)
        columns_form = ArticleColumnForm()
        return render(request, "article/column/article_column.html", {"columns": columns,"column_form":columns_form})
    if request.method == "POST":
        columns_name = request.POST['column']
        columns = ArticleColumn.objects.filter(user_id=request.user.id,column=columns_name)
        if columns:
            return HttpResponse('2')
        else:
            ArticleColumn.objects.create(user=request.user,column=columns_name)
...
def article_list(request):
    ...
    return render(request,'article/column/article_list.html',{"article":articles})
...
def redit_article(request,article_id):
    if request.method=="GET":
        ...
        return render(request,"article/column/redit_article.html",{"article_columns":article_columns,
                        "article":article,"this_article_form":this_article_form,"this_article_column":this_article_column})
